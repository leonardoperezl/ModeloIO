...
def visualizar_resultados(modelo, datos, tiempos):
    try:
        resultados = pd.DataFrame({
            'Ingrediente': ['Semillas de trigo', 'Semillas de sandía', 'Semillas de calabaza', 'Estofado sospechoso'],
        ...
        plt.title('Composición de la mezcla óptima')
        plt.axis('equal')
        ...
        plt.title('Tiempos de ejecución por etapa')
        plt.ylabel('Segundos')
        plt.xticks(rotation=45)
        ...
        print(f"Gráficas generadas correctamente")
    
    except Exception as e:
        print("Error al generar las gráficas:", e)
        raise
